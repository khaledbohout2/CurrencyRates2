...
extension DataRequest {
    ...
    func response<T: Decodable>(_ dumb: T.Type, queue: DispatchQueue = .main, completionHandler: @escaping (DataResponse<T>) -> Void) -> Self {
        return self.responseDecodable(queue: queue, completionHandler: completionHandler)
    }
    
    @discardableResult
    func responseSwiftCairoUser(queue: DispatchQueue = .main, completionHandler: @escaping (DataResponse<Currencies>) -> Void) -> Self {
        return self.responseDecodable(queue: queue, completionHandler: completionHandler)
    }
}
